...
export function buildJiraAdfBody(summary: RunSummary, issueKey: string, allKeys: string[]): object {
  ...
  // Header paragraph
  const headerText =
    `${icon} Agentic AI Pipeline Report\n\n` +
    `Story : ${issueKey}\n` +
    `Branch: ${process.env.GITHUB_BRANCH ?? 'agent/auto-tests'}${testedWith}\n\n` +
    `CI Result : ${summary.conclusion.toUpperCase()}\n` +
    `Pass rate : ${passRate}% (${summary.passed}/${summary.totalTests} tests)\n` +
